...
# Restores applications settings if available with mackup.
if brew ls --versions mackup > /dev/null; then
  ln -s "${LIB_DIR}/mackup/mackup.cfg" "${XDG_CONFIG_HOME}/mackup/.mackup.cfg" &&
  ln -s "${XDG_CONFIG_HOME}/mackup/.mackup.cfg" "${HOME}"
  if [[ -d "$LIB_DIR/mackup/settings" ]]; then
    if [[ -z "$(ls -A $LIB_DIR/mackup/settings | head -n 5)" ]]; then
      echo 'mackup restore not possible the directory is empty!'
    else
      mackup restore
    fi
  fi
